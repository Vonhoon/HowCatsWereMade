...
public class GravityBlock : MonoBehaviour
{
    ...
    {       
        if (other.CompareTag("PlayerBlock"))
        {
            ...
            {
                other.GetComponent<PlayerBlock>().player.GetComponent<Player>().noGrav = true;
            ...
            if (other.GetComponent<DragScript>().dragging)
            {
                if (connected.Contains(other.gameObject))
                {
                    ...
                    other.GetComponent<PlayerBlock>().player.GetComponent<Player>().noGrav = false;
                }
                EnableLine();
            }
        ...
        if (other.CompareTag("ObstacleBlock"))
        {
            if (!GetComponent<DragScript>().dragging && !other.GetComponent<DragScript>().dragging)
            {
                ...
                {                        
                    other.GetComponent<ObstacleBlock>().obstacleList[i].GetComponent<Obstacle>().noGrav = true;                        
                }
                if (!connected.Contains(other.gameObject))
                {
                    connected.Add(other.gameObject);
    ...
    private void ChangeGravityScale(Collider2D other)
    {
        for (int i = 0; i < other.GetComponent<ObstacleBlock>().obstacleList.Count; i++)
        {
            other.GetComponent<ObstacleBlock>().obstacleList[i].GetComponent<Obstacle>().noGrav = false;
        }
    }
}
